...
public class FaceSDK {
    ...
    private static Algorithm sAlgorithm;
    ...
    public static boolean init(Context context) {
        sContext = context;
        if ("201607".equals(new SimpleDateFormat("yyyyMM").format(new Date()))) {
            Constants.isDebugLib = true;
        } else {
            Constants.isDebugLib = false;
        }
        ...
        sAlgorithm = Algorithm.getInstance(context);
    ...
    public static int match(String desF, String srcF) {
        ...
        return Algorithm.Match(srcFeature, desFeature, 0, Algorithm.ENV_SET);
    }
    ...
    public static Stfaceattr decodeBitmap(byte[] data, int width, int height, int orientation) {
        ...
        FaceSDK.mDecodeStatus = sAlgorithm.DiscoverX(hFattr, faceRgb24, hWdHi[0],
                hWdHi[1], 0, Algorithm.ENV_SET);// 换到正常的RGB空间后，就可以检测DiscoverX了。
