...
import attendance.yn.a606a.receiver.USBDiskReceiver;
...
public class Excel {
    ...
    public Excel(Context context) {
        ...
        makeDir(file);
        if (!USBDiskReceiver.UsbFile.equals("")) {
            usbFile = new File(USBDiskReceiver.UsbFile + "/YAST");
            makeDir(usbFile);
        }
    }
    ...
    public void initData() {
        if (!USBDiskReceiver.UsbFile.equals("")) {
            ...
            ExcelUtils.initExcel(usbFile.toString() + "/考勤记录表" + time + ".xls", title);
            ExcelUtils.writeObjListToExcel(dbManager.getBillData(), usbFile.toString() + "/考勤记录表" + time + ".xls", context);
        }
        String time = formatter.format(new Date());
    ...
    public void initUserData() {
        if (!USBDiskReceiver.UsbFile.equals("")) {
            String time = formatter.format(new Date());
            ExcelUtils.initExcel(usbFile.toString() + "/人员记录表" + time + ".xls", userTitle);
            ExcelUtils.writeObjListToExcel(dbManager.getUserData(), usbFile.toString() + "/人员记录表" + time + ".xls", context);
        }
        String time = formatter.format(new Date());
