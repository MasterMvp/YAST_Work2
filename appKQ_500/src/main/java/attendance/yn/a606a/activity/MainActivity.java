...
import com.YinanSoft.phoneface.util.ToastUtil;
...
import attendance.yn.a606a.adapter.Main2Adapter;
...
//    private static MyApplication mApp;
    ...
    private boolean isExitVaildFinger = true;
...
//                    Main2Activity.fingerExt = new Main2Activity.fingerExt(MainActivity.this);
//                initFingerPrinter();
//            }
//        }).start();
    ...
    private void initCamera() {
        ...
        //--------------------------------------------------------------------
        //uvc摄像头初始化
        if (!Build.MODEL.toUpperCase().contains("JWZD-500")) {//手动录入核查暂时不打开UVC
...
//            Toast.makeText(UVCCameraActivityOther.this, "USB_DEVICE_DETACHED", Toast.LENGTH_SHORT).show();
    ...
    public void onDecodeSuc(Result obj) {
        try {
            ...
            Log.e(TAG, "mSurfaceView.startPreviewDelay(0);出错了");
        }
...
//        }
        if (Build.MODEL.toUpperCase().toString().equals("JWZD-500")) {
            matchBitmap(bitmap, mBlackFeatureList, false);
        } else {
            matchBitmap(bitmap, mBlackFeatureList, true);
        }
...
//                isMatching = false;
        ...
        if (!"".equals(featureL)) {
            ...
            View view = LayoutInflater.from(MainActivity.this).inflate(R.layout.dialog_show_userinfo, null);
...
//            }
            isMatching = false;
//            isExitVaildFinger = false;
//            handler.post(runableValidFinger);
        }
...
//            Log.e("MianActivity", "isExitVaildFinger:" + isExitVaildFinger);
            while (!isExitVaildFinger) {
                Log.e("MianActivity", "111111111111isExitVaildFinger:" + isExitVaildFinger);
                //采集指纹图像和特征值
...
//                    bitmapFinger = BitmapFactory.decodeByteArray(Main2Activity.fingerExt.fingerprint.imageBuffer, 0, Main2Activity.fingerExt.fingerprint.imgSize);
                    ...
                    for (int i = 0; i < mFingerFeatureList.size(); i++) {
                        ...
                        Log.e(TAG, (i + 1) + "次指纹比对。");
                        System.arraycopy(Base64.decode(featuresF), 0, f1, 0, 512);
                        System.arraycopy(Base64.decode(featuresF), 512, f2, 0, 512);
                        //特征值比对
